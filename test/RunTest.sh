...
#  kill -9 $upid 2>/dev/null
 ...
 # terminate alice in a softest possible way to prevent loss of output
  sleep 1
  kill -SIGINT "$apid" 2>/dev/null
  if ps -p "$apid" >/dev/null; then
    sleep 1
    kill -SIGTERM "$apid" 2>/dev/null
    if ps -p "$apid" >/dev/null; then
      sleep 1
      kill -9 "$apid" 2>/dev/null
    fi
  ...
  # terminate unrelinet in a softest possible way to prevent loss of output
  sleep 1
  kill -SIGINT "$upid" 2>/dev/null
  if ps -p "$upid" >/dev/null; then
    sleep 1
    kill -SIGTERM "$upid" 2>/dev/null
    if ps -p "$upid" >/dev/null; then
      sleep 1
      kill -9 "$upid" 2>/dev/null
    fi
  ...
  if [[ $added -ne 1 ]]; then
    ...
    res=${PIPESTATUS[0]}
    # move results to the tmp dir
    diff Alice.txt Alice-in.txt
    res2=${PIPESTATUS[0]}
    diff Bob.txt Bob-in.txt
    res3=${PIPESTATUS[0]}
    tput rev;
    if [[ $res == "0" ]];
    then
      ...
      then
        ...
        then
          ...
          echo "Failed: Data is correct but Bob has inconsistent corrupted rate with UnreliNET"
        fi
      else
        echo "Failed: Data is correct but Alice has inconsistent corrupted rate with UnreliNET"
      fi
    else
